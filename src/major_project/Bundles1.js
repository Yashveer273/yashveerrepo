...
function  Bundles1() {
  ...
  const value = [{ email, mobil_no, card_no,cardDate,...pay,selects,orderTime,pincode,address,username},...cart1];
  
  const submit = (e) => {
    e.preventDefault();
// localStorage.setItem("a",JSON.stringify(cart1))
 setOrderTime(+'orderDate-'+mydate.getFullYear()+'/'+(mydate.getMonth()+1)+'/'+mydate.getDate()+':  Order Time-'+mydate.getHours()+'-'+mydate.getMinutes())
payment()
...
console.log(value)
  ...
  useEffect(()=>{
    fetch("http://localhost:5000/abc2").then((Result) => {
...
<div>
        ...
        {P2.map((item) => (
            <div key={item._id} className="sq1" style={{float:"left"}}>
              ...
              <span className="description" style={{marginLeft:"-29.5px"}}>  {item.name}{item.price}{item.star}{item.Reviews}
      ...
      <div>
        ...
        >
          ...
          <ul style={{ flexDirection: "column" }} className="AllProduct">
            ...
            >
              ...
              <Link to="/Personal_care" style={{ color: "black", lineHeight: "3rem" }}>
                <div> Personal Care</div>
              </Link>
              <Link to="/Bundles" style={{ color: "black", lineHeight: "3rem" }}>
                <div> Bundles</div>
              </Link>
        ...
        <div className="cart" style={{maxWidth:"27%"}}>
         ...
         {cart1.map((item) => (
             <div key={item._id} className="sq4 " 
